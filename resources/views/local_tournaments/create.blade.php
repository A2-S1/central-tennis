...
@section('content')
<div class="container">
  <h1>Cadastrar Torneio Local</h1>
  <form method="POST" action="{{ route('local_tournaments.store') }}" enctype="multipart/form-data" class="row g-3 mt-2">
    @csrf
    <div class="col-md-6">
    ...
    <div class="col-md-6">
      <div class="form-check mb-2">
        <input class="form-check-input" type="checkbox" name="registration_is_free" value="1" id="regfree" {{ old('registration_is_free') ? 'checked' : '' }}>
        <label class="form-check-label" for="regfree">Inscrição gratuita</label>
      </div>
      <label class="form-label">Valor inscrição (R$)</label>
      <input type="number" step="0.01" name="registration_fee" class="form-control @error('registration_fee') is-invalid @enderror" value="{{ old('registration_fee') }}">
      @error('registration_fee')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-md-6">
      <div class="form-check mb-2">
        <input class="form-check-input" type="checkbox" name="ticket_is_free" value="1" id="tickfree" {{ old('ticket_is_free', true) ? 'checked' : '' }}>
        <label class="form-check-label" for="tickfree">Ingressos gratuitos</label>
      </div>
      <label class="form-label">Valor do ingresso (R$)</label>
      <input type="number" step="0.01" name="ticket_price" class="form-control @error('ticket_price') is-invalid @enderror" value="{{ old('ticket_price') }}">
      @error('ticket_price')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-12">
      <button class="btn btn-success">Salvar</button>
      <a href="{{ route('local_tournaments.index') }}" class="btn btn-outline-secondary">Cancelar</a>
    </div>
  </form>
</div>
@endsection
